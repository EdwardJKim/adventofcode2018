...
in X,Y format where X and Y are integers and zero or greater. (Adjacent regions 
...
The erosion level is (16807 + 510) % 20183 = 17317. The type is 17317 % 3 = 1, 
wet.
At 0,1, because the X coordinate is 0, the geologic index is 1 * 48271 = 48271. 
The erosion level is (48271 + 510) % 20183 = 8415. The type is 8415 % 3 = 0, 
rocky.
At 1,1, neither coordinate is 0 and it is not the coordinate of the target, so 
the geologic index is the erosion level of 0,1 (8415) times the erosion level 
of 1,0 (17317), 8415 * 17317 = 145722555. The erosion level is (145722555 + 
...
class Cave:
    ...
    def __init__(self, depth: str, target: str, buf: int = 1000) -> None:

        self.depth = int(depth)
        self.target_x, self.target_y = map(int, target.split(','))

        self._map = [
            [-1] * (self.target_y + buf)
            for _ in range(self.target_x + buf)
        ]
        self._el_mem = [
            [-1] * (self.target_y + buf)
            for _ in range(self.target_x + buf)
    ...
    def geologic_index(self, x: int, y: int) -> int:
        ...
        if self._gi_mem[y][x] != -1:
            return self._gi_mem[y][x]
    ...
    def region_type(self, x: int, y: int) -> int:
    ...
    def _fill_map(self) -> None:
        ...
        for i in range(m):
            for j in range(n):
                if self._map[i][j] == -1:
                    self._map[i][j] = self.region_type(j, i)
    ...
    def draw_map(self) -> List[str]:
        ...
        result = [[symbols[i] for i in row] for row in self._map]
        result[0][0] = 'M'
        result[self.target_y][self.target_x] = 'T'

        return [
            ''.join(row[:self.target_x + 1])
            for row in result[:self.target_y + 1]
        ]
    ...
    def risk_level(self) -> int:
        ...
        return sum(
            sum(row[:self.target_x + 1])
            for row in self._map[:self.target_y + 1]
        )
