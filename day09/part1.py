...
def play(num_players: int, last_marble: int) -> int:
    ...
    while count <= last_marble:
        ...
        current_idx = (current_idx + 1) % len(marbles) + 1
        if count % 23 == 0:
            scores[player] += count
            current_idx = (current_idx - 9 + len(marbles)) % len(marbles)
            scores[player] += marbles.pop(current_idx)
        else:
            marbles.insert(current_idx, count)
        count += 1
