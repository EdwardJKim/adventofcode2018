...
def read_input(filename: str) -> List[Tuple[datetime, str]]:
    ...
    with open(filename) as f:
        for line in f:
            ...
            records.append((dt, action))

    records.sort(key=lambda x: (x[0].month, x[0].day, x[0].hour, x[0].minute))

    return records


def process(records: List[Tuple[datetime, str]]) -> Dict[int, List[int]]:

    counter: Dict[int, List[int]] = dict()

    for dt, action in records:
        if "begins shift" in action:
            guard_id = int(action.lstrip("Guard #").rstrip(" begins shift"))
            if guard_id not in counter:
                counter[guard_id] = [0] * 60
...
def find_most_asleep_guard(records: Dict[int, List[int]]) -> int:

    minutes_asleep = [
        (guard, sum(minutes)) for guard, minutes in records.items()
    ]
    minutes_asleep.sort(key=lambda x: -x[1])
    return minutes_asleep[0][0]


def find_most_asleep_minute(record: List[int]) -> int:
...
def test_read_input():
    ...
    records = read_input("test.txt")
    assert all(isinstance(dt, datetime) for dt, action in records)
    assert all(isinstance(action, str) for dt, action in records)
    assert all(dt1 < dt2
        for (dt1, _), (dt2, _) in zip(records[:-1], records[1:]))


def test_process():

    records = read_input("test.txt")
    counter = process(records)
    assert counter[10][24] == 2
    assert sum(counter[10]) == 50
    assert sum(counter[99]) == 30


def test_find_most_asleep_guard():

    records = process(read_input("test.txt"))
    assert find_most_asleep_guard(records) == 10


def test_find_most_asleep_minute():

    records = process(read_input("test.txt"))
    guard = find_most_asleep_guard(records)
    assert find_most_asleep_minute(records[guard]) == 24
...
if __name__ == "__main__":
    ...
    test_read_input()
    test_process()
    test_find_most_asleep_guard()
    test_find_most_asleep_minute()
    print("all tests passed.")

    records = process(read_input("input.txt"))
    guard = find_most_asleep_guard(records)
    minute = find_most_asleep_minute(records[guard])
